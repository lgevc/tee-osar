					    ...
					    uint32_t max_key_size)
{
	...
	if (res != TEEC_SUCCESS) {
		errx(1, "TA_CRYPT_CMD_ALLOCATE_OPERATION failed with code 0x%x origin 0x%x", res, ret_orig);
	}
						   ...
						   TEE_ObjectHandle *obh)
{
	...
	if (res != TEEC_SUCCESS) {
		errx(1, "TA_CRYPT_CMD_ALLOCATE_TRANSIENT_OBJECT failed with code 0x%x origin 0x%x", res, ret_orig);
	}

	if (res == TEEC_SUCCESS)
		*obh = (TEE_ObjectHandle)(uintptr_t)op.params[1].value.a;

	return res;
}

void tee_add_attr(size_t *attr_count, TEE_Attribute *attrs, uint32_t attr_id,
						   ...
						   uint32_t attr_count)
{
	...
	res = tee_pack_attrs(attrs, attr_count, &buf, &blen);
	if (res != TEEC_SUCCESS)
		return res;
	...
	if (res != TEEC_SUCCESS && res != TEEC_ERROR_TARGET_DEAD) {
		errx(1, "TA_CRYPT_CMD_POPULATE_TRANSIENT_OBJECT failed with code 0x%x origin 0x%x", res, ret_orig);
	}
					  ...
					  TEE_ObjectHandle key)
{
	...
	if (res != TEEC_SUCCESS) {
		errx(1, "TA_CRYPT_CMD_SET_OPERATION_KEY failed with code 0x%x origin 0x%x", res, ret_orig);
	}
					       ...
					       TEE_ObjectHandle obh)
{
	...
	if (res != TEEC_SUCCESS) {
		errx(1, "TA_CRYPT_CMD_FREE_TRANSIENT_OBJECT failed with code 0x%x origin 0x%x", res, ret_orig);
	}
						     ...
						     void *buf, size_t *blen)
{
	...
	if (res != TEEC_SUCCESS) {
		errx(1, "TA_CRYPT_CMD_GET_OBJECT_BUFFER_ATTRIBUTE failed with code 0x%x origin 0x%x", res, ret_orig);
	}
					...
					TEE_OperationHandle oph)
{
	...
	if (res != TEEC_SUCCESS) {
		errx(1, "TA_CRYPT_CMD_FREE_OPERATION failed with code 0x%x origin 0x%x", res, ret_orig);
	}
